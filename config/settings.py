"""Django settings for the config-checker project.

All deployment-specific values are read from environment variables (or a
``.env`` file) via python-decouple, so the same code runs in development and
on the production server without edits.
"""
...
# Hosts allowed to submit the form (needed behind https / a domain).
...
# بدون Manifest: اگر collectstatic کامل نباشد ادمین به‌هم نمی‌ریزد.
# USE_FINDERS یعنی CSS ادمین حتی قبل از collectstatic هم سرو می‌شود.
STORAGES = {
